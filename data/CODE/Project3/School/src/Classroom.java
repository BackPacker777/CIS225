 ...
 * @see ASSIGNMENT: Looping
 * @see PURPOSE: Demonstrate looping
 */
...
public class Classroom {
	...
	public void makeChair(String color, boolean hasWheels) {
		seats[counter] = new Chair(color, hasWheels);
		counter++;
	}
	...
	public void printPersonAge(int personType, int person) {
		final int STUDENT = 0;
		if (personType == STUDENT) {
			System.out.println("" + students.get(person).getAge());
		} else {
			System.out.println("" + teachers.get(person).getAge());
		}
	}
	...
	public void removeHuman(int personType) {
		final int STUDENT = 0;
		Random person = new Random();
		if (personType == STUDENT) {
			students.remove(person.nextInt(students.size()));
		} else {
			teachers.remove(person.nextInt(teachers.size()));
		}
	}
